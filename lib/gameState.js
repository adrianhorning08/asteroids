...
class GameState extends State {
  ...
  handleInputs(input) {
    if (this.gameOver && input.isDown('enter')) {
      this.game.nextState = States.GAME;
      return;
    }
    if (!this.ship.visible && input.isDown('spacebar') && !this.gameOver) {
    ...
    if (input.isPressed('spacebar') && !this.gameOver) {
  ...
  render(ctx) {
    ...
    if (this.gameOver) {
      ctx.vectorText('game over', 10, null, 200);
      ctx.vectorText('Your score was ' + this.points, 5, null, 300);
      ctx.vectorText('Press enter to play again', 2, null, 400);
    }
